...
class MedicationModal extends React.Component<IProps, IState> {
    ...
    return (
      <View style={CommonStyles.screenContainer}>
        ...
        <View style={{flexDirection: 'row', marginHorizontal: 5}}>
          <Text style={[CommonStyles.text, {flexGrow: 1}]}>Время приёма</Text>
          {!this.state.readonly ? 
  ...
  private save() {
    ...
    if (this.props.medications.some(t => t.title.toLowerCase() == 
...
const mapState = (state: RootState) => ({
  ...
  current: state.medications.current,
  lastId: state.medications.lastId
});

const mapDispatch = (dispatch: AppDispatch) => ({
  setCurrent: (m: Medication) => dispatch(setCurrent(m)),
  push: (m: Medication) => dispatch(push(m)),
  remove: (m: Medication) => dispatch(remove(m))
})
